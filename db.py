...
async def init_db():
    ...
    async with _pool.acquire() as conn:
        ...
        # already exists from an earlier deploy, so patch them in explicitly.
        await conn.execute("""
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS user_stats (
                ...
                total_type_disabled INTEGER NOT NULL DEFAULT 0
            )
        """)
        await conn.execute("""
            ALTER TABLE user_stats
                ADD COLUMN IF NOT EXISTS total_type_disabled INTEGER NOT NULL DEFAULT 0
        """)
...
async def get_stats(user_id: int) -> dict:
    async with _pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            "total_size_filtered, total_type_disabled FROM user_stats WHERE user_id = $1",
            user_id,
        )
        if row is None:
            return {"total_processed": 0, "total_duplicates": 0,
                     "total_near_duplicates": 0, "total_size_filtered": 0,
                     "total_type_disabled": 0}
        return dict(row)
...
async def reset_stats(user_id: int):
    async with _pool.acquire() as conn:
        await conn.execute(
            "UPDATE user_stats SET total_processed = 0, total_duplicates = 0, "
            "total_near_duplicates = 0, total_size_filtered = 0, "
            "total_type_disabled = 0 WHERE user_id = $1",
            user_id,
        )
